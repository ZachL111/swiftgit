...
struct TestRunner {
    static func main() {
        ...
        let domainReview = DomainReview(signal: 48, slack: 20, drag: 32, confidence: 46)
        precondition(DomainReviewLens.score(domainReview) == 66)
        precondition(DomainReviewLens.lane(domainReview) == "hold")
    }
}
